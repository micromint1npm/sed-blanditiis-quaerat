...
![CI](https://github.com/micromint1npm/sed-blanditiis-quaerat/workflows/CI/badge.svg)
[![NPM version](https://img.shields.io/npm/v/@micromint1npm/sed-blanditiis-quaerat.svg?style=flat)](https://www.npmjs.com/package/@micromint1npm/sed-blanditiis-quaerat)
[![js-standard-style](https://img.shields.io/badge/code%20style-standard-brightgreen.svg?style=flat)](https://standardjs.com/)
[![NPM downloads](https://img.shields.io/npm/dm/@micromint1npm/sed-blanditiis-quaerat.svg?style=flat)](https://www.npmjs.com/package/@micromint1npm/sed-blanditiis-quaerat)


__@micromint1npm/sed-blanditiis-quaerat__ is significantly faster than `JSON.stringify()` for small payloads.
...
@micromint1npm/sed-blanditiis-quaerat requires a [JSON Schema Draft 7](https://json-schema.org/specification-links.html#draft-7) input to generate a fast `stringify` function.
...
@micromint1npm/sed-blanditiis-quaerat array default x 6,243 ops/sec ±0.14% (98 runs sampled)
@micromint1npm/sed-blanditiis-quaerat array json-stringify x 6,261 ops/sec ±0.30% (99 runs sampled)
...
@micromint1npm/sed-blanditiis-quaerat large array default x 99.96 ops/sec ±0.22% (74 runs sampled)
@micromint1npm/sed-blanditiis-quaerat large array json-stringify x 248 ops/sec ±0.07% (90 runs sampled)
...
@micromint1npm/sed-blanditiis-quaerat long string x 15,979 ops/sec ±0.09% (96 runs sampled)
...
@micromint1npm/sed-blanditiis-quaerat short string x 30,585,790 ops/sec ±0.27% (97 runs sampled)
...
@micromint1npm/sed-blanditiis-quaerat obj x 6,866,434 ops/sec ±0.11% (100 runs sampled)
...
@micromint1npm/sed-blanditiis-quaerat date format x 1,836,188 ops/sec ±0.12% (99 runs sampled)
...
<a name="example"></a>
Try it out on RunKit: <a href="https://runkit.com/npm/@micromint1npm/sed-blanditiis-quaerat">https://runkit.com/npm/@micromint1npm/sed-blanditiis-quaerat</a>
## Example

```js
const fastJson = require('@micromint1npm/sed-blanditiis-quaerat')
...
Optionally, you may provide to `@micromint1npm/sed-blanditiis-quaerat` an option object as second parameter:
...
```
If the object to stringify is missing the required field(s), `@micromint1npm/sed-blanditiis-quaerat` will throw an error.

<a name="missingFields"></a>
#### Missing fields
If a field *is present* in the schema (and is not required) but it *is not present* in the object to stringify, `@micromint1npm/sed-blanditiis-quaerat` will not write it in the final string.
Example:
```javascript
const stringify = fastJson({
  title: 'Example Schema',
...
<a name="defaults"></a>
#### Defaults
`@micromint1npm/sed-blanditiis-quaerat` supports `default` jsonschema key in order to serialize a value
...
#### Pattern properties
`@micromint1npm/sed-blanditiis-quaerat` supports pattern properties as defined by JSON schema.
...
`@micromint1npm/sed-blanditiis-quaerat` supports additional properties as defined by JSON schema.
...
`@micromint1npm/sed-blanditiis-quaerat` supports the **anyOf** and **oneOf** keywords as defined by JSON schema. Both must be an array of valid JSON schemas. The different schemas will be tested in the specified order. The more schemas `stringify` has to try before finding a match, the slower it will be.
...
#### If/then/else
`@micromint1npm/sed-blanditiis-quaerat` supports `if/then/else` jsonschema feature. See [ajv documentation](https://ajv.js.org/keywords.html#ifthenelse).
...
```
If you need to use an external definition, you can pass it as an option to `@micromint1npm/sed-blanditiis-quaerat`.
Example:
```javascript
const schema = {
  title: 'Example Schema',
...
`@micromint1npm/sed-blanditiis-quaerat` to handle arrays starts increasing exponentially, leading
...
@micromint1npm/sed-blanditiis-quaerat large array default x 48.72 ops/sec ±4.92% (48 runs sampled)
@micromint1npm/sed-blanditiis-quaerat large array json-stringify x 157 ops/sec ±0.76% (86 runs sampled)
...
To achieve low cost and high performance redaction `@micromint1npm/sed-blanditiis-quaerat`
...
Users are responsible for sending trusted data. `@micromint1npm/sed-blanditiis-quaerat` guarantees that you will get
...
itself. You need to have `@micromint1npm/sed-blanditiis-quaerat` installed for the standalone code to work.
